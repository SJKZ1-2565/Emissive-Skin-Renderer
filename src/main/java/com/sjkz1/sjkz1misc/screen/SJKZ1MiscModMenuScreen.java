	  ...
	  private Screen createConfigScreen(Screen screen)
	    {
			...
			generalCategory.addEntry(entry.startIntSlider(new TranslatableComponent("sjkz1misc.config.y_hp_detector"), config.yPositionHealthStatus,1,3).setTooltip(new TextComponent("Y position of health status.")).setSaveConsumer(value -> config.yPositionHealthStatus = value).setDefaultValue(1).build());
			generalCategory.addEntry(entry.startBooleanToggle(new TranslatableComponent("sjkz1misc.config.tamed_horse_detector"), config.showTamedHorse).setTooltip(new TextComponent("Show tame data.")).setSaveConsumer(value -> config.showTamedHorse = value).setDefaultValue(false).build());
			generalCategory.addEntry(entry.startIntSlider(new TranslatableComponent("sjkz1misc.config.y_tamed_horse_detector"), config.yPositionHorseDisplay,1,3).setTooltip(new TextComponent("Y position of display tamed horse")).setSaveConsumer(value -> config.yPositionHorseDisplay = value).setDefaultValue(2).build());
